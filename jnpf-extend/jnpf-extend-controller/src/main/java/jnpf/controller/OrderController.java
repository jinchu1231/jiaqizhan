...
import jnpf.model.order.OrderCustomerVO;
import jnpf.model.order.OrderEntryListVO;
import jnpf.model.order.OrderExportModel;
import jnpf.model.order.OrderForm;
import jnpf.model.order.OrderGoodsVO;
...
import jnpf.model.order.OrderListVO;
import jnpf.model.order.OrderReceivableListVO;
...
public class OrderController extends SuperController<OrderService, OrderEntity> {
    ...
    public ActionResult<PageListVO<OrderListVO>> list(PaginationOrder paginationOrder) {
        ...
        List<String> userId = listVO.stream().map(t -> t.getCreatorUserId()).collect(Collectors.toList());
        ...
        for (OrderListVO order : listVO) {
            ...
            FlowTaskEntity task = orderStaList.stream().filter(t -> t.getId().equals(order.getId())).findFirst().orElse(null);
            order.setCurrentState(task != null ? task.getStatus() : null);
    ...
    public ActionResult<OrderInfoVO> info(@PathVariable("id") String id) {
        ...
        if (infoVo != null) {
            ...
            if (StringUtil.isNotEmpty(infoVo.getCreatorUserId())) {
            ...
            if (StringUtil.isNotEmpty(infoVo.getLastModifyUserId())) {
    ...
    @SaCheckPermission("extend.order")
    public ActionResult create(@PathVariable("id") String id, @RequestBody @Valid OrderForm orderForm) throws WorkFlowException {
