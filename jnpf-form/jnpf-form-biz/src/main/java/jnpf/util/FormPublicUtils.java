...
import jnpf.model.form.MultipleControlEnum;
...
public class FormPublicUtils {
    ...
    public static Map<String, Object> getDataMap(List<Map<String, Object>> dataList, FieLdsModel swapModel) {
        PropsModel propsModel = swapModel.getProps();
        Map<String, Object> dataInterfaceMap = new HashMap<>();
        dataList.stream().forEach(data -> {
            dataInterfaceMap.put(String.valueOf(data.get(propsModel.getValue())), String.valueOf(data.get(propsModel.getLabel())));
        });
     ...
     * @param isFlow
     */
    public static List<Map<String,Object>> addIdToList(List<Map<String,Object>> dataList,String key,boolean isFlow){
        return dataList.stream().map(data -> {
            data.put(FlowFormConstant.ID , data.get(key));
            if (isFlow) {//有开启流程需要查询流程引擎id
                Object flwoId = null;
                if(data.get(TableFeildsEnum.FLOWID.getField())!=null){
                    flwoId = data.get(TableFeildsEnum.FLOWID.getField());
                }
                if(data.get(TableFeildsEnum.FLOWID.getField().toUpperCase())!=null){
                    flwoId = data.get(TableFeildsEnum.FLOWID.getField().toUpperCase());
                }
                data.put(FlowFormConstant.FLOWID ,  String.valueOf(flwoId));
            }
            return data;
