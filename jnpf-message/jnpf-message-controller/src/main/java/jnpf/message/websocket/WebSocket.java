...
import jnpf.message.util.ConnectionType;
import jnpf.message.util.MessageChannelType;
import jnpf.message.util.MessageParameterEnum;
import jnpf.message.util.OnlineUserModel;
import jnpf.message.util.OnlineUserProvider;
import jnpf.message.util.SendMessageTypeEnum;
...
//                            jsonObject.put("title",userInfo.getUserName() + "/"+userInfo.getUserAccount());
            ...
            case "MessageList":
                ...
                List<ImContentEntity> data = imContentService.getMessageList(sendUserId, receiveUserId, pageModel)
                        .stream().sorted(Comparator.comparing(ImContentEntity::getSendTime)).collect(Collectors.toList());
